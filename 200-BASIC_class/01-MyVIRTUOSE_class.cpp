...
class MyVIRTUOSE {        // The class
    ...
    public:              // Access specifier
        const char* PORT;                                       // myAttribute1
        float forcefactor;                                      // myAttribute2
        float speedfactor;                                      // myAttribute3
        float dt; // sampling rate - VERY IMPORTANT             // myAttribute4
        ...
        void debug_getPOS() {
            for (int i = 0; i < 10; i++) {
                getPOS();
                std::cout << myPOS[0] << myPOS[1] << myPOS[2] << myPOS[3] << myPOS[4] << myPOS[5] << myPOS[6] << std::endl;
            }
        }
...
int main() {
    ...
    /////////////////////////////////////////// Constructor 1
    //MyVIRTUOSE RightARM("127.0.0.1#53210", myFORCEFACTOR, mySPEEDFACTOR, myDT);       // Use constructor 1 to create an object of MyVIRTUOSE class.
    //RightARM.quick_start();
    //RightARM.set_dt(0.1f); // if you'd like to change the dt, // choices include // //0.0001f
    //RightARM.debug_getPOS();
    //RightARM.quick_stop();
    //myObj.testrun();

    /////////////////////////////////////////// Constructor 2
    //MyVIRTUOSE RightARM("127.0.0.1#53210");                                             // Use constructor 2 to create an object of MyVIRTUOSE class.
    //RightARM.connect();
    //
